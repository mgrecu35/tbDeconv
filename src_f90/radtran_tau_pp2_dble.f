...
!
      SUBROUTINE RADTRAN_pp(UMU, NLYR, tbout, BTEMP, LYRTEMP, LYRHGT, 
     $                  KEXT,
     $                  SALB, ASYM, KEXTu,
     $                  SALBu, ASYMu, FISOT, EMIS, EBAR, ilambert)
C
C     CHRIS KUMMEROW
...
!f2py real, intent(out) :: tbout
      ...
      INTEGER NLYR, I, J, NANG, NN, ilambert
      REAL  UMU, BTEMP, LYRTEMP(0:NLYR), LYRHGT(0:NLYR), 
     $      KEXT(NLYR), SALB(NLYR), ASYM(NLYR), KEXTu(NLYR), 
     $      SALBu(NLYR), ASYMu(NLYR)
...
C
      ...
      if(.not. delta) then
        do i = 1, nlyr
          kext_delta(i) = dble(kextu(i))
          salb_delta(i) = dble(salbu(i))
          asym_delta(i) = dble(asymu(i))
        end do
      else
        do i = 1, nlyr
          fdelta = dble(asymu(i) * asymu(i))
          kext_delta(i) = dble((1. - salbu(i) * fdelta) * kextu(i))
          salb_delta(i) = dble((1. - fdelta) * salbu(i) /
     $     (1. - salbu(i) * fdelta))
          asym_delta(i) = dble(asymu(i) / (1. + asymu(i)))
        end do
      end if
      DO 101  J = 1,NLYR
